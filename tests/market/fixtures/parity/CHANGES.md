...
- **2026-08-19 15:48 CT** (14 events + 1 absorption, base 13339bc): st-q5xu: the upside mirror is named — a resistance engagement is failed_breakout / level_reject (bearish), no longer failed_breakdown with bias=bearish; invalidation word at a resistance is 'held above, no reject'
- **2026-08-26 01:17 CT** (14 events + 1 absorption, base 517e102): st-bkvt: sweep reason renders from the lexicon emission schema; ticks_swept now displays as tick-level on both surfaces (review finding 1)
- **2026-08-27 12:40 CT** (14 events + 1 absorption, base f23481c): SweepPrint gains span_ms and concentration [2026-08-27] — the two fields that make its docstring's 'one aggressor, near-instantly' checkable. Additive: the engine golden's hash reproduces the prior pin byte-for-byte once the two fields are stripped, and every pre-existing key in this snapshot is unchanged.
