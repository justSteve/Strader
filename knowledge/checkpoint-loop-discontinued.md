...
**How to apply:** Do not start `/loop 30m /checkpoint` at session start (the session-start.sh hook mandate was removed the same day). The checkpoint skill itself no longer exists either — its `Skill(checkpoint)` allow rule was dropped 2026-09-05 as a rule for a tool that is not there (9791a83), and this line said otherwise until 2026-09-06 [st-rfjg]. `/handoff` (snapshot.json) remains the real state-capture ritual.
