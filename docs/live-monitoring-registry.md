...
| Is the ES tick tape being captured and advancing? | `scripts/capture_health.py` (pure assessor `strader/capture_health.py`) | Only surface that separates *process alive* from *process receiving*; drives the `*/2` supervisor and the 08:25 pre-open heartbeat. Writes `data/corpus/_capture_health.json`. | `surface_liveness.sh` "ES capture" row — tells you a process matched, not that bytes are landing. Use it for presence, not for health. |
| Is the GexBot 10-endpoint feed alive? | `data/corpus/_gexbot_health.json` (written by `cron/gexbot-supervisor-session.sh` → shared `capture-supervisor-wrapper.sh`) | Same assessor, its own state file; knows the RTH gate and the NYSE holiday table, so it can say `idle` instead of crying `dead` all weekend. | `surface_liveness.sh` "GEX collector" row — presence only, no staleness verdict. |
| Is the 1 Hz orderflow leg alive? | `data/corpus/_gexbot_of1s_health.json` (`cron/gexbot-orderflow-1s-supervisor.sh`) | Separate process, separate state file by design — a shared file would cross the two collectors' cycle counts and cry stale every two minutes. | `surface_liveness.sh` "GEX 1Hz orderflow" row (added under this bead) — presence only. |
...
### 2.1 Supervised live tier (cron restarts them; DOWN inside the window is a fault)

| Surface | Question | Reads | Writes | Started by | State |
|---|---|---|---|---|---|
| `corpus_stream_databento.py` | live ES trades + MBP-1 | Databento live | `data/corpus/<day>/databento_glbx_es*.jsonl` | `cron/capture-supervisor-session.sh` `*/2`, window 02:50–15:05 CT | **LIVE** (pid 775145, relaunched 02:50 today — `restarts:1`) |
| `corpus_poll_gexbot.py` | 10 GexBot endpoints @60s | GexBot API | `gexbot.jsonl` | `cron/gexbot-supervisor-session.sh` `*/2`, 08:30–15:05 CT, holiday-aware | DOWN — **normal**, pre-window |
| `corpus_poll_gexbot_orderflow_1s.py` | orderflow spike train @~1 Hz | GexBot `/SPX/orderflow/orderflow` | `gexbot_orderflow_1s.jsonl` | `cron/gexbot-orderflow-1s-supervisor.sh` `*/2`, same gate | DOWN — **normal**, pre-window (13 MB written 08-12) |
