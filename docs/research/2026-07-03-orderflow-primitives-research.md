...
To put it another way: almost every trade arrives with a tag saying "a buyer forced this" or "a seller forced this," and delta is just the running difference between those two piles. But a handful of trades arrive with no tag at all — because of *how* they happened, nobody crossed the spread. The opening auction matches a crowd of orders at one price; a halt resumes the same way; some trades are triggered by the exchange's own implied orders. Real contracts changed hands, but there is genuinely no "aggressor" to name. Those untagged trades cluster at exactly the noisy moments — the open, the restart after a pause — which is when a trader most wants to glance at delta and trust it. So the rule has to be written down before the first line of delta code: untagged volume goes in its own bucket, visible but never added to either pile. If instead the code shrugs and counts them as zero (or worse, defaults them to one side), the delta line starts the day already lying, and every read built on it inherits the lie.

Now the six candidate primitives. Each ends with a **"v1 verdict"** — the admission decision for version one of the orderflow engine. The charge each verdict answers: *does this primitive earn a place in the first build, and in what role?* Four tests, applied to every candidate: **computable** (from data we hold — trades now, quotes after Phase B), **deterministic** (replaying the same recorded day gives the identical answer), **trustworthy** (established practice, not invented thresholds dressed up as signals), and **needed** (the Q2 setup signatures actually consume it). A primitive can pass with a reduced role — admitted as slow-moving context rather than a per-tick signal, or as a graded score rather than a firm flag — and the verdicts say which.
