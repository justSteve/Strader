...
and ripped." Elsewhere in the same letter the elevator move is "from 7465 down
...
## Act I — the trap, on film this time (08:30 → 10:16)
...
08:49:23 (confidence 0.8). Fifty-nine seconds later the market runs it over. At
...
contracts (97.9th / 99.2nd, F1 strong-band 0.958), 08:51 adds −6.75 (F1 0.934).
...
0.912). The recognizer confirms the 7447 failed_breakdown twice more — 09:21:23
...
13.0 points (20% of the 65.0-point final range), and the entire morning trap —
down 23.25 summed-net points, back up — lives *inside* leg 1. The excursion
...
trap resolution lives at atom level; the leg tier supplies only regime. The
...
08:33:05, with the tape near 7443, the record shows level_reclaim engagements
*forming* at 7474 **and at 7506** — the latter sixty-three points overhead —
...
at all — zero events all day, a silent anchor. And the level the letter's whole
...
1.100, giveback 0.318, force −502 (aligned). At corpus scale it is F4 (effort
12.2nd / effect 6.1st percentile). The cascade lands on **probe-fade** — out,
...
its third-to-last atom (10:18, F2 solid-band 0.348, 4,326 contracts, travel
...
signature's ≥8-point flush-leg requirement. Small trap, small spring.
...
below the steady window, so *steady-leg, off-pace-slow flag*; flip effect down
...
the window edge (34.1), so even the off-pace flag versus core is nearly a coin
flip. The label is not embarrassed; it is telling you exactly how much to lean
...
printed high is *not* at the pivot: RunMeta's range high is 7496.5, and the
...
closed a tick and a half from its extreme. Effect 72.8nd percentile. And its
...
stages — flush 11:47:52, stall 11:51:46, flip 11:54:22, **confirmation-event
11:59:14** — a bullish confirmation fired inside a falling **host-leg**. The
...
The 12:09 pivot-atom is the quiet kind — F3 at 0.108, 1,887 contracts, the
...
aligned — but at corpus scale it is F4 (effort 17.8th / effect 40.4th):
...
back a point deeper, three times slower.
...
reaches flush+flip+stall by 13:36:49, and dies unconfirmed at 14:04:31. The
...
minute, five ticks apart. The 13:42:47 delta-divergence stamps the afternoon
...
the 14:25 pivot-atom, is F4 at 0.348: 1,850 contracts, travel 0.10. The day's
...
extreme in 35 minutes, pace 0.500, giveback 0.186, and the day's most emphatic
aligned force: +4,918 of net buying on 132,514 contracts. At corpus scale:
...
(> 0.3) minimum, and this one is below it — nearest alternative *dead-drift*
across the effort axis, with *probe-fade* only 0.224 away across the giveback
...
with leg-grind)* → absorption-stall *(lean, below the off-diagonal solid bar)*.

Three of seven archetype calls land in the coin-flip grade-band, against a
corpus expectation of ~19% — this day was drawn mostly on the cutlines. The
...
Three archetypes never got a bare label today (two of them lurk as coin-flip
pair-partners; none was assigned). Their best sightings from other days in the
...
- **leg-grind** — the trend-day crawl; it haunted today as legs 1 and 6's
  pair-partner but never won an assignment. Specimen: 2025-10-29, 08:57 —
...
And one homecoming: the **counterforce-leg**, which the 7/22 epilogue could
only show you as a specimen (2026-07-27's −81.75 on net buying), printed live
today as Act IV. The bestiary rotates.
