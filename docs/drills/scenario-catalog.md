...
## Part I — Level-engagement scenarios (the core six)
...
- **What:** Price approaches the level, aggression into it is punished immediately, price
  leaves the way it came. The level holds on first contact.
- **Footprint:** Approach columns show fading intensity into the level; at the touch, one
  or two bright cells against the approach direction; footers flip color at the level.
...
- **Call:** Reject. (In guess-then-reveal terms: price bounces ≥4 points away.)
- **Reference:** [to tag] — pin from 7/2 guess-then-reveal visits on a rejected touch.

### S2 · Failed breakdown (the trap)

- **What:** Sellers break the level with real force, the break stops paying, buyers take
  over, price retakes the level. The full four-stage sequence with a violent flush —
  Mancini's signature setup, bullish.
- **Footprint:** Stage 1 flush — bright red cells THROUGH the level, red footers, fast
  ...
  the level on blue pressure.
- **Lookalike:** S4 (clean break). The tell is stage 2: in S2 the sellers stop getting
  paid below the level; in S4 the break keeps producing lower prices at pace.
- **Call:** After confirm: accept the reclaim (long bias). Before confirm: hands off.
...
- **What:** Same destination as S2 — price ends up back above a lost level with opposite
  evidence — but the flush lacked violence; the level was lost quietly rather than trapped.
- **Footprint:** The give-through is dim (thin, low-effort red), then the retake carries
  the same flip/confirm evidence as S2. Flush violence is the family separator the
  recognizer itself uses.
...
- **Footprint:** Bright red through the level and the lows KEEP coming — footers stay
  red, pace stays fast, each bar's POC steps lower. No stage 2.
...
- **What:** The pattern gets to stage 2 or 3 — sellers stall, delta even flips — but the
  retake never comes; price grinds back down through the would-be reclaim. The absorber
  gave up or was overrun.
- **Footprint:** A stall and flip form, then blue pressure fails at the level from below;
  footers revert red; the recognizer's lifecycle ends `invalidated`.
...
- **What:** Price oscillates across the level with conviction in neither direction.
  The level is noise today, or the tape is resting on it.
- **Footprint:** Alternating footer colors, dim cells both sides of the level, slow
  bars, POCs clustered ON the level rather than stepping away from it.
- **Lookalike:** S1 early on. Tell: S1 leaves the level after rejecting; S6 keeps
