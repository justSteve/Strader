...
## Addendum A — Strader's 2026-08-19 inputs (st-g1u7, memo `COO/docs/a2a/2026-08-19-strader-to-coo-postmortem-inputs.md`)

Folded in before the build started; the executor extends the tasks named. Each item is a measurement Strader did by hand on the 08-19 session and wants the ledger to carry.

**A1 — anchor kind from the parse, and the `kind-mismatch` flag (extend Tasks 5 and 8).** The recognizer emits `anchor_kind: support` for every Mancini level (st-tme: the kind is dropped in `anchors.py`). Today's two worst confirms were on 7742 and 7758, both *major resistance* in the parse. In `measure_calls`, add `anchor_kind_parse` to every SetupRecognition row: look the anchor price up in `runbook/mancini/parsed/<day>.json` → `levels[*].kind` (exact price match; `None` when the parse has no such level; the module takes the parsed levels as an argument, `analyze_day(..., parsed_levels=...)`, loaded by the CLI with a try/except so a missing parse never stops the run). New flag `kind-mismatch` in `flags()`: a `confirmed` setup whose `anchor_kind_parse` is `resistance` (rule: parse says resistance, recognizer said support). Test: a row with `anchor_kind_parse="resistance"` trips; `"support"` and `None` do not. Page: the kind shows in the "What it said" column when the parse disagrees (`@ 7742 (parse: resistance)`).

**A2 — the anchorless run, said outright (extend Task 8 and the page).** On 08-19 the feeder restarted at the 00:00 rollover before the 04:11 parse landed, so the run carried zero Mancini anchors until 07:34. A run whose header `mancini` list is empty gets `anchorless: true` in `res["runs"][i]`, and the page header says: "Run N carried no Mancini levels (HH:MM → HH:MM) — no calls there is not nothing to call." Test on a Segment with `meta["mancini"] == []`.

**A3 — lid and absorption on every leg's origin row (extend Task 6; two knobs).** Strader's named setup (`knowledge/reclaim-under-the-lid.md`) separates today's one good 7738 confirm from five false ones by two bar-measurable facts before the move. On every leg row add:
- `lid_rejections` — count of bars in the **M minutes** before the origin whose high landed within **K ticks** under the nearest level (`level − K·0.25 ≤ high < level`), for bullish legs; mirrored (`level < low ≤ level + K·0.25`) for bearish legs; `None` when no level is within `z_pts`.
- `window_delta` — sum of bar `d` over the same M minutes, and `window_px_change` — close at origin minus close M minutes earlier. Absorption reads as delta positive while price went nowhere; the page shows the two numbers, it does not name it.
Knobs in `config/postmortem.yaml`: `lid_ticks: 8`, `lid_window_min: 30`. Tests: a hand-built path with four highs at 7737.75–7738.00 under a 7738 anchor in the window → `lid_rejections == 4`; `window_delta` sums the fixture deltas. The backfill summary (Task 12) adds one table: ±5 outcome at 30 min for `called`/`hinted` bullish legs with `lid_rejections ≥ 3` versus `< 3` — the measurement Strader most wants from the 306 days.

**A4 — his word against the machine's word (extend Task 7).** Mancini keeps "reclaim" and "Failed Breakdown" apart; the recognizer assigns `failed_breakdown` vs `level_reclaim` by flush intensity, not by what the level was (all six 7738 confirms today were one or the other by flush depth). The FAMILY tier already matches the sibling; add `word_match: true|false` to every matched recap row (`his setup == machine setup`) and a count on the page: "N of M of his named setups were called by the other name." No new tier.

**A5 — not ours: the gate and the recall tool.** (i) The plain-words gate rewrote Steve's quoted words inside frontmatter ("as distinct as Mancini's FBD"): a `"…"` quoted-span exclusion goes to co-1np7q, not here. (ii) `tools/recall.py` over claude-monitor's `conversation_entries` table: Strader offered to take it — accepted on the inbox; COO adds the pointer to the tap-in/memory conventions ("for 'what did Steve say about X' the monitor DB, not the repo transcripts").

Spec §3a/§3b/§3d/§9 are amended by this addendum; the spec file carries a one-line pointer to it.
