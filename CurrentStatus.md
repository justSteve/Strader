...
0d. **Prune tranche 1 (`st-rfjg`) EXECUTED and closed 2026-09-06.** 136 files left HEAD; `pre-prune-2026-09-05` holds every one. Five audit rows were corrected by measurement before the delete, and step 4 was struck by Desk after `strader/feeds/__init__.py` measured with zero importers and 65 lines — neither its prune verdict nor its F4 survived, and the seam is now a tranche-4 decision. `st-2opj` and `st-sl1f` closed with it. The 09-03 outage aftermath (`st-e12g`) is unchanged and still open: overnight/evening ES windows unfilled, DNS/network root cause unknown.

0e. **CI on master was red for 25 days and is now green.** First break 2026-08-12 `bb598f0`; 95 of the last 100 runs failed. CI installs neither `databento` nor `schwab-py` by design and **one collection error aborts the whole run**, so a single unimportable module hid the entire suite while local runs stayed green. Fixed at causes (`st-v55j`, `7b9b0d5`): 1,662 passed / 58 skipped on both 3.11 and 3.12. **The green is 58 skips wide** — three of the four second-round fixes make CI skip rather than run, which is an environment gap now visible instead of hidden. `st-efdu` filed to fail a handoff loudly when master's latest run is not green, scoped off any trading-day path.

0e. **The six-story taxonomy names both sides — `st-z1a1` Mirror The Taxonomy, CLOSED 2026-09-10.** Steve, 08-20: "i notice failed breakdown but no failed breakout. perhaps our 6-way story needs work?" The stories were written support-side only while the recognizer had emitted the upside mirror since 08-19 — 47% of its confirms (1,077 of 2,312 on run `20260819T213124Z`) had no home in the canon. Now: direction is an explicit axis in `docs/drills/scenario-catalog.md` Part I, `scenario-deck.json` (every S ref carries `side`/`kind`), `skill-ladder.md` and `docs/foundation/07-levels-and-traps.md`; every story names and examples its ↓ (support, bullish) and ↑ (resistance, bearish) instance; **17 ↑ references pinned from fresh replay records** (07-02, 07-20, and 08-19 added to the deck deliberately as the up day); the asymmetry is written into the canon (midday depression is bullish-only, p = 0.008; sides never pooled). **S1 renamed "Clean hold"** (was "Clean rejection") so "reject" belongs to one story — the recognizer's `level_reject` is S3 ↑; recorded in `docs/lexicon/lexicon.yaml` as Strader's input for Desk to clear, judgment-button verb left to st-g9y. Unblocks st-fpc4 (Structural Grade Rebuild). Side finding filed as **`st-r88d` Deck Refs Drift**: the 07-02 ↓ refs sit at 7511/7506, prices the current anchor rule does not produce (parse: 7512/7502), and the old 07-20 S5 ref @7522 was a resistance read as support — moved to S2 ↑.

0f. **Schwab gate execd credential paths — `st-hun6` LANDED and CLOSED.** Steve landed the behavioural half 2026-09-08 09:06 (`9f558c1`: gate 4 covers the execd credential paths, gate 6 blocks the two minters); the 09-09 tap-in pinned it on measurement (six execd BLOCK cases, five mention controls in `tests/test_schwab_gate_hook.py`). Nothing on this item waits on Steve. Original 09-08 text follows. **The schwab gate is blind to the execd credential paths — `st-hun6`, waiting on Steve.** Gate 4 is keyed on `tokens/` alone and there is no gate 6, so six commands that write or mint a Schwab credential still return ALLOW: `cp` and redirect into `/var/lib/execd/market.json`, `chmod` on it, `cp` over `/etc/execd/vault.json`, and running either credential minter. Re-measured 2026-09-06 after Steve landed the comment half (`331f224`); the behavioural half did not land. Patch regenerated against the live hook and applies clean (`docs/patches/`, `-p0`); COO added `.test`/`.commit` sidecars so `land-patch.sh` runs it in one command (`4f7838e`). Hooks are Steve's to land.
...
2d. **Code Estate Plan correction** (`st-nujt`): the census claim of a
   ...
   Related live state: **steves-desk DOWN from the 2026-09-08 19:05 CT reboot, restored by COO 2026-09-09 08:00 CT — 8 windows, observed UP at the 09-10 tap-in** (`st-p4d6` closed on that observation; boot-time restore of the moocity socket is still unanswered on the Desk memo). Older: steves-desk has only the hand-rebuilt Trading window —
