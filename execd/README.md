...
**PAPER/LIVE from the account page** (co-8mb1z, Steve 2026-09-25: *"support
moving between paper and live without need to re-run the installer"*). Each
instance holds both sides in a `ModeSwitch` (`execd/paper.py`): on Schwab the
paper book and the Trader API, on Alpaca its paper and live venues. The switch
on `/exec/account` (and `/exec-alpaca/account`) takes the passphrase going
live and one tap going back, takes effect at once, and is written to
`<state-dir>/mode`. **Precedence:** `<state-dir>/mode` (the page's switch) >
`/etc/execd*/mode` (Steve's seed, read-only to the service) > `paper`. After a
flip the orders, the paper book, the journal's `mode`, `/status` and the
PAPER/LIVE badge (repainted by the order page's poll) all follow. **The one
refusal:** a flip waits until the instance holds nothing and has nothing
working — a paper position cannot become a live one; FLATTEN is on the page.
On Alpaca, going live without `ALPACA_LIVE_*` keys in the vault says so and
stays on paper. The STAND DOWN button is gone from every screen (the route and
state remain, unused by the page); FLATTEN shows only while a position is open.
